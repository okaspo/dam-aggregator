...
│   ├── build_index.py          # インデックス構築
│   ├── archive_old_data.py     # データアーカイブ
│   └── data_stats.py           # データ統計表示
└── README.md
```
...
データは自動的に`public/data/history/`に蓄積されます：
- 各ダムごとに `.ndjson` ファイル
- 1行1レコード形式
- 重複データは自動でスキップ
...
90日以上経過したデータは自動的に月次アーカイブに移動されます：
...
- 📅 90日以上のデータを自動アーカイブ
- 🗜️ gzip圧縮（約1/10のサイズ）
- 📁 月次ファイル（`{dam_id}_YYYY-MM.ndjson.gz`）
- ⏰ 毎月1日に自動実行（GitHub Actions）
- 💾 アーカイブは`public/data/archive/`に保存
